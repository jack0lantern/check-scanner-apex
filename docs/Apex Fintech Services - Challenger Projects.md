...
Design and build a minimal end-to-end mobile check deposit system that allows investors to deposit checks into brokerage accounts via a mobile application. The system integrates with an external Vendor Service for check image capture, validation, and verification, routes validated deposits through a Funding Service middleware for business rule enforcement and ledger posting, and settles approved deposits with a Settlement Bank. The solution must handle the full lifecycle ”capture, validation, compliance gating, operational review, ledger posting, settlement, and return/reversal scenarios” with a clear operator workflow, audit trail, and stubbed vendor integration that supports differentiated responses for comprehensive scenario testing.
...
- Vendor API stub providing differentiated responses for check image validation scenarios (IQA pass, IQA fail ”blur, IQA fail” glare, MICR read failure, duplicate detected, amount mismatch, clean pass)
...
- **Happy path works end-to-end:** Investor submits check ’Vendor API captures image’ Vendor Service validates â†’ Funding Service applies business rules â†’ ledger posting created ’operator can review’ settlement file generated â†’ deposit marked completed
...
- **Transfer state machine is complete:** Deposits transition correctly through: Requested â†’ Validating ’Analyzing’ Approved ’FundsPosted’ Completed (happy path) and Requested â†’ Validating ’Rejected / Returned (non-happy paths)
...
- Stub must support the following differentiated response scenarios:
  ...
  - **Duplicate Detected**” check has been previously deposited, reject
  - **Amount Mismatch**” OCR amount differs from user-entered amount, flag for review
  - **Clean Pass**” all checks pass, return extracted MICR data, amounts, and transaction ID
...
- Per-deposit decision trace: inputs ’Vendor Service response’ business rules applied ’operator actions’ settlement status
...
7. **Build the demo interface**
   - CLI or minimal UI to simulate: deposit submission ’validation’ approval ’posting’ settlement
