...
/**
 * Stub implementation of {@link VendorService}. Returns deterministic responses based on {@code
 * accountId} (e.g. X-Account-Id). Scenario is selected without code changes by using account IDs
 * that map to each scenario.
 *
 ...
 *
 * <p>Marked {@code @Primary} so it is the default implementation. Swap by providing another {@link
 * VendorService} bean and annotating it {@code @Primary}, or by using a factory/qualifier.
 */
@Service
@Primary
public class StubVendorService implements VendorService {
  ...
  private static final Map<VendorScenario, String> ACTIONABLE_MESSAGES =
      Map.ofEntries(
          Map.entry(
              VendorScenario.IQA_FAIL_BLUR, "Image too blurry — please retake in better lighting"),
          Map.entry(
              VendorScenario.IQA_FAIL_GLARE, "Glare detected — please move to a darker surface"),
          Map.entry(
              VendorScenario.MICR_READ_FAILURE,
              "Cannot read check routing line — please try again or deposit at a branch"),
          Map.entry(VendorScenario.DUPLICATE_DETECTED, "This check has already been deposited"),
          Map.entry(
    ...
    return switch (scenario) {
      case IQA_PASS ->
          new VendorAssessmentResult(
              ...
              null,
              DEFAULT_RISK_SCORE);
      case IQA_FAIL_BLUR, IQA_FAIL_GLARE ->
          new VendorAssessmentResult(
              scenario, 0.4, null, null, null, actionableMessage, DEFAULT_RISK_SCORE);
      case MICR_READ_FAILURE ->
          new VendorAssessmentResult(
              scenario, 0.6, null, null, enteredAmount, actionableMessage, DEFAULT_RISK_SCORE);
      case DUPLICATE_DETECTED, AMOUNT_MISMATCH ->
          new VendorAssessmentResult(
              scenario,
              DEFAULT_VENDOR_SCORE,
              nextDefaultMicr(),
